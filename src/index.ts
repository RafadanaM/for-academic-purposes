...
import { getData, getPopularData, getLatestData, translate } from "./function";
import { firstWordRegex, nuclearCodeRegex } from "./regexes";
dotenv.config();
...
`,
    ...
    case "message":
      ...
      const split = text.split(firstWordRegex);
      switch (split[0]) {
        case "/translate":
          const translatedMessage: TextMessage = {
            type: "text",
            text: translate(split[1]),
          };
          return client.replyMessage(event.replyToken, translatedMessage);
        case "/mantap":
...
More to cum!!`,
        ...
        case (text.match(nuclearCodeRegex) || {}).input: {
